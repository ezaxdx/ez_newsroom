...
export async function GET(req: NextRequest) {
  ...
  // 오늘 요일 확인 (0=Sun, 1=Mon, ..., 6=Sat, KST = UTC+9)
  const nowKST = new Date(Date.now() + 9 * 60 * 60 * 1000);
  const todayDay = nowKST.getUTCDay();

  if (todayDay !== settings.send_day) {
    return NextResponse.json({ skipped: true, reason: `today is ${todayDay}, configured is ${settings.send_day}` });
  }
  ...
  const miceNews: NewsCard[] = (miceRaw ?? []).map(n => ({ title: n.title, summary: n.summary_short, image_url: n.image_url, url: n.original_url }));
  ...
  const tourismNews: NewsCard[] = (tourismRaw ?? []).map(n => ({ title: n.title, summary: n.summary_short, image_url: n.image_url, url: n.original_url }));
  ...
  const featuredEvents: EventCard[] = (featuredRaw ?? []).map(e => ({ name: e.event_name, start_date: e.start_date, venue: e.venue ?? null, website: e.website ?? null }));
  ...
  const { data: upcomingRaw } = await supabase.from("convention_events")
    ...
    .order("start_date", { ascending: true }).limit(7);

  const upcomingEvents: EventCard[] = (upcomingRaw ?? []).map(e => ({ name: e.event_name, start_date: e.start_date, venue: e.venue ?? null, website: e.website ?? null }));

  const html = generateNewsletterHTML({
    vol_number, send_date,
    editorial_text: settings.default_editorial ?? "",
    mice_news: miceNews, tourism_news: tourismNews,
