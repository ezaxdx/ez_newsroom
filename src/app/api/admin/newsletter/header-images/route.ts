...
export const DEFAULT_HEADER_IMAGE = { label: "기본", url: "/images/ez-letter-header.png" };
...
export async function POST(req: NextRequest) {
  ...
  const existing: { label: string; url: string }[] = Array.isArray(settings.newsletter_header_images)
    ? settings.newsletter_header_images : [];
  const updated = [...existing.filter((e) => e.label !== label), { label, url }];
