...
export const DEFAULT_HEADER_IMAGE = { label: "기본", url: "/images/ez-letter-header.png", flap_url: undefined as string | undefined, box_color: undefined as string | undefined };
...
export async function GET() {
  ...
  const extra: { label: string; url: string; flap_url?: string; box_color?: string }[] = Array.isArray(settings?.newsletter_header_images)
...
export async function POST(req: NextRequest) {
  ...
  let body: { label?: string; url?: string; flap_url?: string; box_color?: string };
  ...
  const box_color = body.box_color?.trim() || undefined;
  ...
  const existing: { label: string; url: string; flap_url?: string; box_color?: string }[] = Array.isArray(settings.newsletter_header_images)
    ? settings.newsletter_header_images : [];
  const updated = [...existing.filter((e) => e.label !== label), { label, url, ...(flap_url ? { flap_url } : {}), ...(box_color ? { box_color } : {}) }];
